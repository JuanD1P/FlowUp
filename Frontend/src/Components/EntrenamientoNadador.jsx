...
export default function EntrenamientoNadador() {
  ...
  useEffect(() => {
    ...
    unsubs.auth = onAuthStateChanged(auth, (u) => {
      ...
      if (u?.uid) {
        ...
        unsubs.hist = onSnapshot(qy, (snap) =>
    ...
    const onKey = (e) => {
      if (e.key === "Escape") setShowPulso(false);
    };
  ...
  const setAhoraInicio = () => {
    const n = new Date();
    setForm((p) => ({ ...p, horaInicio: isoHora(n) }));
  };
  const setAhoraFin = () => {
    const n = new Date();
    setForm((p) => ({ ...p, horaFin: isoHora(n) }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const ownerIds = Array.from(new Set([uid, ...coachIds]));
  ...
  const kpis = useMemo(() => {
    ...
    const prevSes = historial.filter((h) => between(h.startMs || 0, 60) && !between(h.startMs || 0, 30))
      .length;
    const rpeProm =
  ...
  const goNext = () => {
    if (canNext) setIdx((i) => Math.min(historial.length - 1, i + 1));
  };
                ...
                >
                  ...
                  <label className="en-field">
                    ...
                    <input
                      ...
                      inputMode="numeric"
                      value={b.series}
                      onChange={(e) => updateBloque(i, "series", Number(e.target.value))}
                    />
                  </label>
                  <label className="en-field en-with-unit">
                    ...
                    <input
                      ...
                      inputMode="numeric"
                      value={b.metrosSerie}
                      onChange={(e) => updateBloque(i, "metrosSerie", Number(e.target.value))}
                    />
                  ...
                  <label className="en-field en-with-unit">
                    ...
                    <input
                      ...
                      placeholder="p.ej. 10"
                      value={b.minutos}
                      onChange={(e) => updateBloque(i, "minutos", e.target.value)}
                    />
          ...
          <section className="en-section">
            ...
            <div className="en-grid">
              <label className="en-field">
                ...
                <input
                  style={{ accentColor: `hsl(${rpeHue} 90% 45%)` }}
                  type="range"
                  min="1"
                  max="10"
                  name="rpe"
            ...
            <label className="en-field en-notas">
              ...
              <textarea
                ...
                value={form.notas}
                onChange={handleChange}
                placeholder="Ej. Últimos 400 m fuertes, técnica estable."
              />
            </label>
          </section>
                  ...
                  {kpis.distSeries.map((v, i) => (
                    <div key={i} className="en-bar" style={{ height: "100%" }}>
                      <div
                        ...
                        style={{ height: `${Math.max(8, Math.round((v / kpis.maxD) * 100))}%` }}
                      />
                      <div className="en-bar-cap">
              ...
              <div className="en-chart">
                ...
                <div className="en-bars en-bars-labeled" style={{ height: 160 }}>
                  {kpis.durSeries.map((v, i) => (
                    <div key={i} className="en-bar" style={{ height: "100%" }}>
                      <div
                        className="en-bar-fill alt"
                        style={{ height: `${Math.max(8, Math.round((v / kpis.maxT) * 100))}%` }}
                      />
                  ...
                  return (
                    <div key={s.id} className="en-tl-item" style={{ padding: 10 }}>
                      <div className="en-tl-head">
                        ...
                        <div
                          className={`en-dot ${
                            s.tipo === "agua" ? "blue" : s.tipo === "tierra" ? "green" : "gray"
                          }`}
                        />
                      </div>
                      ...
                      {Array.isArray(s.bloques) && s.bloques.length > 0 && (
                        <div className="en-tl-nota" style={{ marginTop: 8 }}>
                          ...
                          <ul style={{ margin: 0, paddingLeft: 18 }}>
                            {s.bloques.map((b, ix) => (
                              <li key={ix}>
                                {b.estilo}: {b.series}×{b.metrosSerie} m ={" "}
                                {b.metrosTotales ?? b.series * b.metrosSerie} m
              ...
              <div className="en-modal-col">
                ...
                <div className="en-timer">
                  ...
                  <div className="en-timer-ctrls">
                    <button
                      className={`en-btn ${pulseSecs === 15 ? "en-primary" : ""}`}
                      onClick={() => setPulseSecs(15)}
                      type="button"
                    >
                      15 s
                    </button>
                    <button
                      className={`en-btn ${pulseSecs === 30 ? "en-primary" : ""}`}
                      onClick={() => setPulseSecs(30)}
                      type="button"
                    >
                      30 s
                    </button>
                  ...
                  <div className="en-timer-hint">
                    Cuenta los latidos y multiplica por {pulseSecs === 15 ? 4 : 2}.
                  </div>
                </div>
              </div>
              <div className="en-modal-col">
                ...
                <div className="en-zones">
                  <div className="en-zone">
                    ...
                    <div className="en-zone-bars">
                      <div className="en-zone-bar z1">
                        <span>Suave</span>
                      </div>
                      <div className="en-zone-bar z2">
                        <span>Aeróbica</span>
                      </div>
                      <div className="en-zone-bar z3">
                        <span>Tempo</span>
                      </div>
                      <div className="en-zone-bar z4">
                        <span>Umbral</span>
                      </div>
                      <div className="en-zone-bar z5">
                        <span>Alta</span>
                      </div>
                    </div>
