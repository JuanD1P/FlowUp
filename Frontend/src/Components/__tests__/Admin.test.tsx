...
describe('Admin — Gestión de usuarios', () => {
  ...
  it('eliminar usuario: confirma, ejecuta preConfirm (DELETE) y recarga', async () => {
    const axiosMod: any = (await import('axios')).default;
    const apiMock = axiosMod.__getApiMock();
    ...
    // Click en eliminar de Ana
    const filaAna = screen.getAllByRole('row').find(r => within(r).queryByText('Ana'))!;
    const btnEliminar = within(filaAna).getByRole('button', { name: /eliminar/i });
    ...
    // DELETE correcto
    await waitFor(() => {
      expect(apiMock.delete).toHaveBeenCalledWith('/api/usuarios/u1');
    });

    // Recargó lista: GET 2 veces (carga + recarga tras eliminar)
    await waitFor(() => {
      expect(apiMock.get).toHaveBeenCalledTimes(2);
    });
  });
