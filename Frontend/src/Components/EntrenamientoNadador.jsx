...
function isoFecha(d){const x=new Date(d);return`${x.getFullYear()}-${String(x.getMonth()+1).padStart(2,"0")}-${String(x.getDate()).padStart(2,"0")}`}
function isoHora(d){const x=new Date(d);return`${String(x.getHours()).padStart(2,"0")}:${String(x.getMinutes()).padStart(2,"0")}`}
...
function pace100Str(mins, meters){if(!mins || !meters) return "";const per100 = mins / (meters/100);const mm = Math.floor(per100);const ss = Math.round((per100-mm)*60);return `${mm}:${String(ss).padStart(2,"0")} /100m`;}
...
export default function EntrenamientoNadador(){
  ...
  const kpis=useMemo(()=>{
    ...
    return{dist7d:winA,distTrend:pct(winA,prevA),ses30d:ses30,sesTrend:pct(ses30,prevSes),rpeProm:isFinite(rpeProm)?rpeProm:0,distSeries,durSeries,maxD,maxT}
  },[historial]);
        ...
        {values.map((v,i)=>{
          ...
          return (
            <g key={i} transform={`translate(${x+3},${160-h-10})`}>
              ...
              <text x={bw/2} y={h+12} textAnchor="middle" className="en-bar-lbl">{v ? (v>=1000 ? (v/1000).toFixed(1)+'k' : v) : ''}</text>
            </g>
          )
        })}
      </svg>
        ...
        {values.map((v,i)=>{
          ...
          return (
            <g key={i} transform={`translate(${x+3},${160-h-10})`}>
              ...
              <text x={bw/2} y={h+12} textAnchor="middle" className="en-bar-lbl">{v ? `${v}m` : ''}</text>
            </g>
          )
        })}
      </svg>
...
const exportarPDF = async () => {
  try{
    ...
    const distOf = (s)=> typeof s.distanciaTotal==="number" ? s.distanciaTotal :
          ...
          <section className="en-section">
            ...
            <div className="en-grid">
              ...
              <label className="en-field en-icon">
                ...
                <div className="en-quick"><button type="button" className="en-chip" onClick={setAhoraInicio}>Ahora</button></div>
              </label>
              <label className="en-field en-icon">
                <span>Hora fin</span>
                <input type="time" name="horaFin" value={form.horaFin} onChange={handleChange}/>
                <i className="en-ico clock"/>
                <div className="en-quick"><button type="button" className="en-chip" onClick={setAhoraFin}>Ahora</button></div>
              </label>
          ...
          <section className="en-section">
            ...
            <div className="en-grid">
              <label className="en-field">
                ...
                <div className="en-gauge mini"><div className="en-gauge-fill" style={{width:`${(Number(form.rpe)/10)*100}%`}}/></div>
              ...
              <label className="en-field en-icon">
                ...
                <div className="en-quick"><button type="button" className="en-chip" onClick={()=>{setShowPulso(true);setTimer(0)}}>Cómo medir</button></div>
              </label>
              <div className="en-field">
                <span>Fatiga</span>
          ...
          <div className="en-kpi-card">
            ...
            <div className="en-kpi-grid">
              <div className="en-kpi-item">
                ...
                <div className={`en-trend ${kpis.distTrend>=0?"up":"down"}`}>{kpis.distTrend>=0?"▲":"▼"} {Math.abs(kpis.distTrend).toFixed(0)}%</div>
              </div>
              <div className="en-kpi-item">
                <span className="en-kpi-label">Sesiones 30 días</span>
                <div className="en-kpi-val">{kpis.ses30d}</div>
                <div className={`en-trend ${kpis.sesTrend>=0?"up":"down"}`}>{kpis.sesTrend>=0?"▲":"▼"} {Math.abs(kpis.sesTrend).toFixed(0)}%</div>
              </div>
              <div className="en-kpi-item">
                <span className="en-kpi-label">RPE prom (10)</span>
                <div className="en-kpi-val">{kpis.rpeProm.toFixed(1)}</div>
                <div className="en-gauge mini"><div className="en-gauge-fill" style={{width:`${(kpis.rpeProm/10)*100}%`}}/></div>
              </div>
            </div>
          ...
          <div className="en-card en-resumen en-card-glass">
            <div className="en-resumen-head">
              ...
              <div className="en-head-actions" style={{gap:6}}>
                ...
                <button className="en-btn" type="button" onClick={()=>setVerMas(v=>!v)} disabled={!historial.length}>{verMas?"Ver 1":"Ver más"}</button>
      ...
      {showPulso&&(
        <div className="en-modal" onClick={()=>setShowPulso(false)}>
          <div
            ...
            style={{width:isTiny?'100%':'min(920px,100%)',height:isTiny?'100dvh':'auto',maxHeight:isTiny?'100dvh':'calc(100dvh - 32px)',borderRadius:isTiny?0:18,overflow:'auto'}}
          >
            <div className="en-modal-head">
              <h3>Medir frecuencia cardiaca</h3>
              <button className="en-x" onClick={()=>setShowPulso(false)}>×</button>
            </div>
            <div className="en-modal-body2" style={{display:'grid',gridTemplateColumns:isNarrow?'1fr':'320px 1fr',gap:16,alignItems:'start'}}>
              <div className="en-modal-col">
                <img className="en-modal-img" src={MODAL_IMG} alt="Guía para medir el pulso"/>
                <div className="en-timer">
                  <div className="en-timer-display">{String(Math.floor(timer/60)).padStart(2,"0")}:{String(timer%60).padStart(2,"0")}</div>
